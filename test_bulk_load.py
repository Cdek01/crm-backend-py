...
# --- НАСТРОЙКИ ---
BASE_URL = "http://89.111.169.47:8005"
# Укажите ваш токен для регистрации
...
def run_test():
    # --- Шаг 1: Авторизация ---
    print("--- 1. Авторизация ---")
    user_email = f"bulk_loader_all_{UNIQUE_ID}@example.com"
    password = "password123"
    reg_payload = {"email": user_email, "password": password, "registration_token": CORRECT_REGISTRATION_TOKEN}
    requests.post(f"{BASE_URL}/api/auth/register", json=reg_payload).raise_for_status()
    ...
    # --- Шаг 2: Массовая загрузка Юридических лиц ---
    print("\n--- 2. Массовая загрузка Юридических лиц ---")
    legal_entities_to_load = [
        {"short_name": "ООО Ромашка", "inn": f"7701{UNIQUE_ID % 1000000:06}"},
        {"short_name": "АО Вектор", "inn": f"7702{UNIQUE_ID % 1000000:06}"},
    ]
    response = requests.post(f"{BASE_URL}/api/legal-entities/bulk-load", headers=headers, json=legal_entities_to_load)
    response.raise_for_status()
    count = response.json().get("created_count")
    print_status(count == len(legal_entities_to_load), f"Успешно загружено {count} юр. лиц.")

    # Проверка
    list_response = requests.get(f"{BASE_URL}/api/legal-entities/", headers=headers)
    print_status(len(list_response.json()) == len(legal_entities_to_load), "Количество юр. лиц в БД совпадает.")

    # --- Шаг 3: Массовая загрузка Физических лиц ---
    print("\n--- 3. Массовая загрузка Физических лиц ---")
    individuals_to_load = [
        {"full_name": f"Иванов Иван {UNIQUE_ID}"},
        {"full_name": f"Петров Петр {UNIQUE_ID}"},
        {"full_name": f"Сидоров Сидор {UNIQUE_ID}"},
    ]
    response = requests.post(f"{BASE_URL}/api/individuals/bulk-load", headers=headers, json=individuals_to_load)
    response.raise_for_status()
    count = response.json().get("created_count")
    print_status(count == len(individuals_to_load), f"Успешно загружено {count} физ. лиц.")

    # Проверка
    list_response = requests.get(f"{BASE_URL}/api/individuals/", headers=headers)
    print_status(len(list_response.json()) == len(individuals_to_load), "Количество физ. лиц в БД совпадает.")
