...
def process_file_import_task(file_path: str, user_id: int, tenant_id: int, import_config: dict):
    ...
    try:
        ...
        # --- ГЛАВНОЕ ИЗМЕНЕНИЕ: НАДЕЖНАЯ ОЧИСТКА ДАННЫХ ---
        # 1. Принудительно конвертируем ВСЕ данные в строки, чтобы избежать проблем с типами.
        df = df.astype(str)
        # 2. Заменяем строковые представления NaN и NaT (которые создает pandas) на None.
        #    None будет корректно проигнорирован сервисом EAV.
        df.replace({'nan': None, 'NaT': None}, inplace=True)
        # --------------------------------------------------

        df.rename(columns=column_mappings, inplace=True)

        # Старая очистка больше не нужна, так как мы сделали более надежную выше
        # df_cleaned = df.replace({np.nan: None, pd.NaT: None})

        print(f"-> Начинается импорт {len(df)} строк в таблицу '{new_entity_type.display_name}'...")

        imported_count = 0
        for index, row in df.iterrows():
            entity_data = {
                sys_name: row[sys_name] for orig_header, sys_name in column_mappings.items()
                if sys_name in row and pd.notna(row[sys_name])
            }
        ...
        print(f"-> Успешно обработано и импортировано {imported_count} строк.")
        ...
        import traceback
        traceback.print_exc()
        # Важно пробросить ошибку, чтобы Celery пометил задачу как FAILED
        raise e
    finally:
        if db:
            db.close()
