...
# --- Глобальные переменные для теста ---
test_failed = False
NEW_TABLE_NAME = f"import_excel_test_{int(time.time())}"
TEST_EXCEL_FILENAME = "test_import_data.xlsx"  # <-- Меняем расширение файла
...
# --- Основные функции теста (с изменениями) ---

def create_test_excel():
    """Создает временный Excel-файл (.xlsx) для теста."""
    data = [
        {"Ref ID": "ID-001", "Компания": "ООО \"Альфа, Гамма\"", "Дата сделки": "15.01.2024", "Выручка": 150000.50,
         "Кол-во лицензий": 10, "Активен": "true", "Рейтинг (1-5)": 5, "Комментарий": "Все хорошо, оплата прошла"},
        {"Ref ID": "ID-002", "Компания": "ИП Сидоров А.Б.", "Дата сделки": "20/11/2023", "Выручка": 85000,
         "Кол-во лицензий": 5, "Активен": "Да", "Рейтинг (1-5)": 4, "Комментарий": ""},
        {"Ref ID": "ID-003", "Компания": "ЗАО 'Вектор'", "Дата сделки": "2024-03-30", "Выручка": None,
         "Кол-во лицензий": 1, "Активен": "no", "Рейтинг (1-5)": 3, "Комментарий": "Нужно перезвонить"},
        {"Ref ID": "ID-004", "Компания": "ОАО \"Техно-Строй\"", "Дата сделки": "01 Aug 2022", "Выручка": 320000.00,
         "Кол-во лицензий": 25, "Активен": "1", "Рейтинг (1-5)": None, "Комментарий": ""},
        {"Ref ID": "ID-005", "Компания": "Интернет-магазин \"Все для дома\"", "Дата сделки": "10-02-2024",
         "Выручка": 999.90, "Кол-во лицензий": 1, "Активен": "Yes", "Рейтинг (1-5)": 5, "Комментарий": "Срочный заказ"}
    ]
    df = pd.DataFrame(data)
    # index=False КРАЙНЕ ВАЖНО, чтобы pandas не добавлял свою колонку с индексами (0, 1, 2...) в Excel-файл
    df.to_excel(TEST_EXCEL_FILENAME, index=False)
    print_status(True, f"Тестовый файл '{TEST_EXCEL_FILENAME}' (Excel) создан.")


def run_import_test(headers: Dict[str, str]) -> Optional[int]:
    """Выполняет полный цикл импорта Excel-файла."""
    created_table_id = None
    try:
        # --- Шаг 1: Загрузка файла ---
        print_header("Шаг 1: Загрузка Excel-файла на сервер")
        url_upload = f"{BASE_URL}/api/imports/upload"

        with open(TEST_EXCEL_FILENAME, 'rb') as f:
            # Указываем правильное имя файла и MIME-тип для .xlsx
            files = {
                'file': (TEST_EXCEL_FILENAME, f, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
            response_upload = requests.post(url_upload, headers=headers, files=files)
        ...
        print_status(ok,
                     f"Файл успешно загружен. Получен file_id: {file_id}. Распознано колонок: {len(headers_from_file)}")
        if not ok: return None
        ...
        import_config = {
            "new_table_name": NEW_TABLE_NAME,
            "new_table_display_name": "Клиенты из Excel",
            "columns": []
        }
        for header_info in headers_from_file:
            original_header = header_info['original_header']
            do_import = "Комментарий" not in original_header  # Не импортируем "Комментарий"

            import_config["columns"].append({
                "original_header": original_header,
                "display_name": original_header,
                "value_type": header_info['suggested_type'],
                "do_import": do_import
            })
        ...
        response_process = requests.post(url_process, headers=headers, json=import_config)
        response_process.raise_for_status()
        print_status(True, f"Фоновая задача на импорт Excel успешно запущена.")

        # --- Шаг 3: Ожидание и проверка результата ---
        print_header("Шаг 3: Ожидание и проверка результата")
        print("-> Ждем 50 секунд, пока Celery обработает файл...")
        time.sleep(50)
        ...
        if new_table_meta:
            ...
            print_status(False, f"Тест провален: таблица '{NEW_TABLE_NAME}' не была создана Celery.", tables_data)
            return None
        ...
        total_rows = imported_data.get('total', 0)
        ok_rows_count = total_rows == 5
        print_status(ok_rows_count, f"В новой таблице создано {total_rows} строк (ожидалось 5).")
...
def cleanup(headers: Dict[str, str], table_id: Optional[int]):
    """Удаляет временный Excel и созданную таблицу."""
    if os.path.exists(TEST_EXCEL_FILENAME):
        os.remove(TEST_EXCEL_FILENAME)
        print_status(True, f"Временный файл '{TEST_EXCEL_FILENAME}' удален.")

    # ... (остальной код очистки без изменений)
...
def main():
    ...
    created_table_id = None
    create_test_excel()  # <-- Вызываем новую функцию
    try:
        created_table_id = run_import_test(headers)
    finally:
        # cleanup(headers, created_table_id)

        print_header("Итоги тестирования")
        if test_failed:
            print("❌ Тестирование импорта Excel завершилось с ошибками.")
            sys.exit(1)
        else:
            print("✅ Тест импорта Excel прошел успешно!")
