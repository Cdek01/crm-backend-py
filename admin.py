...
class EntityTypeAdmin(ModelView, model=models.EntityType):
    ...
    list_query_options = [
        joinedload(models.EntityType.tenant),
    ]
...
# ... (Классы PermissionAdmin, RoleAdmin, AssignRoleView остаются без изменений)
class PermissionAdmin(ModelView, model=models.Permission):
    ...
    async def delete_model(self, request: Request, pk: str) -> None:
        ...
        if model and model.name in self.PROTECTED_PERMISSIONS:
            raise HTTPException(status_code=400, detail=f"Нельзя удалить системное разрешение '{model.name}'.")
...
class SharedAccessAdmin(ModelView, model=models.SharedAccess):
    ...
    # 2. Указываем, как форматировать связанные поля (чтобы видеть email и имена)
    column_formatters = {
        "grantor": grantor_formatter,
        "grantee": grantee_formatter,
        "entity_type": entity_type_formatter
    }
    ...
    form_columns = [
        models.SharedAccess.grantor,
        models.SharedAccess.grantee,
        models.SharedAccess.entity_type,
        models.SharedAccess.permission_level,
    ]

    # 5. Улучшаем UX для выпадающих списков, чтобы они не загружали тысячи записей
    # (Опционально, но крайне рекомендуется для больших баз данных)
    form_ajax_refs = {
        'grantor': {
