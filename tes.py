...
def register_and_login():
    unique_id = int(time.time())
    email = f"bulk_deleter_{unique_id}@example.com"
    password = "password123"
    reg_payload = {"email": email, "password": password, "full_name": "Bulk Deleter",
                   "registration_token": CORRECT_REGISTRATION_TOKEN}
    requests.post(f"{BASE_URL}/api/auth/register", json=reg_payload).raise_for_status()
    auth_payload = {'username': email, 'password': password}
    token = requests.post(f"{BASE_URL}/api/auth/token", data=auth_payload).json()['access_token']
...
def run_bulk_delete_test():
    try:
        ...
        table_name = f"tasks_bulk_delete_{int(time.time())}"
        ...
        print_status(True, f"Создана тестовая таблица '{table_name}'.")

        # --- ШАГ 2: НАПОЛНЕНИЕ ДАННЫМИ ---
        print_header("ШАГ 2: СОЗДАНИЕ 5 ТЕСТОВЫХ ЗАПИСЕЙ")

        ids_to_delete = []
        ids_to_keep = []

        # Получаем ID всех существующих записей, чтобы найти новую
        initial_list_resp = requests.get(f"{BASE_URL}/api/data/{table_name}", headers=headers)
        existing_ids = {item['id'] for item in initial_list_resp.json()}

        # Создаем 3 задачи на удаление
        for i in range(3):
            payload = {"title": f"Задача на удаление #{i + 1}"}
            resp = requests.post(f"{BASE_URL}/api/data/{table_name}", headers=headers, json=payload)
            response_list = resp.json()

            # --- ИСПРАВЛЕНИЕ: Находим ID, которого не было раньше ---
            all_new_ids = {item['id'] for item in response_list}
            new_id = (all_new_ids - existing_ids).pop()
            ids_to_delete.append(new_id)
            existing_ids.add(new_id)  # Обновляем множество существующих ID
            # ----------------------------------------------------

        # Создаем 2 задачи, которые должны остаться
        for i in range(2):
            payload = {"title": f"Задача, которая останется #{i + 1}"}
            resp = requests.post(f"{BASE_URL}/api/data/{table_name}", headers=headers, json=payload)
            response_list = resp.json()
        ...
        print(f" -> ID для удаления: {ids_to_delete}")
        print(f" -> ID, которые останутся: {ids_to_keep}")
        print_status(True, "5 тестовых записей успешно созданы.")

        # --- ШАГ 3: МАССОВОЕ УДАЛЕНИЕ ---
        print_header("ШАГ 3: ВЫПОЛНЕНИЕ МАССОВОГО УДАЛЕНИЯ")
        ...
        print_status(delete_response.status_code == 200, "Запрос на массовое удаление прошел успешно (статус 200).")
        print_status(
            deleted_count == len(ids_to_delete),
            f"Сервер корректно вернул количество удаленных записей: {deleted_count}."
        )

        # --- ШАГ 4: ФИНАЛЬНАЯ ПРОВЕРКА ---
        print_header("ШАГ 4: ПРОВЕРКА ПОСЛЕДСТВИЙ УДАЛЕНИЯ")

        # Проверка 1: Удаленные записи должны возвращать 404
        print("\n -> Проверяем, что удаленные записи действительно удалены...")
        for deleted_id in ids_to_delete:
            check_url = f"{BASE_URL}/api/data/{table_name}/{deleted_id}"
            check_resp = requests.get(check_url, headers=headers)
            print_status(
                check_resp.status_code == 404,
                f"   -> Запись с ID {deleted_id} недоступна (получен статус 404)."
            )

        # Проверка 2: Оставшиеся записи должны быть доступны
        print("\n -> Проверяем, что нужные записи остались...")
        for keep_id in ids_to_keep:
            check_url = f"{BASE_URL}/api/data/{table_name}/{keep_id}"
            check_resp = requests.get(check_url, headers=headers)
            print_status(
                check_resp.status_code == 200,
                f"   -> Запись с ID {keep_id} на месте (получен статус 200)."
            )

        # Проверка 3: Общий список должен содержать только 2 записи
        print("\n -> Проверяем общий список записей...")
        list_response = requests.get(f"{BASE_URL}/api/data/{table_name}", headers=headers)
        final_list = list_response.json()
        final_ids = {item['id'] for item in final_list}

        print_status(len(final_list) == 2, f"В общем списке осталось {len(final_list)} записи.")
        print_status(set(ids_to_keep) == final_ids, "В общем списке остались правильные ID.")

        print("\n" + "=" * 60)
        print("🎉🎉🎉 ТЕСТ МАССОВОГО УДАЛЕНИЯ ПРОЙДЕН УСПЕШНО! 🎉🎉🎉")
